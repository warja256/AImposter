...
const endVotingRound = async (req, res) => {
    try {
        ...
        if (mostVotedTarget) {
            ...
            const player = await Player.findOne({ where: {id: mostVotedTarget}  })
...
const killVote = async (req, res) => {
    try {
        ...
        if (room.status != "night") {
            return res.status(400).json({ message: 'В комнате не ночь, мафии голосовать нельзя' });
        }
        ...
        //Игрок мафия или нет
        const player = await Player.findOne({ where: { id: voterId, role: 'mafia' } });
        if (!player) {
            return res.status(400).json({ message: 'Голосующий игрок не является мафией' });
        }
...
//Завершить мафиозное голосования и определить убитого игрока
...
module.exports = { votePlayer, endVotingRound, killVote, killPlayer };
